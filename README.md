...
**Manage Habits** opens as a full-screen page in the category currently selected on Home. For example, opening Manage from Hygiene shows **Manage Hygiene** and only Hygiene habits, regardless of whether they are scheduled or complete today; opening it from Home's All tab still opens the protected aggregate. Manage Category uses Home-scale 16px/500 titles and 14px descriptions on separate rows, a compact six-dot grip with a 44px interaction target, and the same placeholder/proxy 250ms vertical long-press reorder behavior as Categories. Icon-only Back controls return to the actual origin: Home-originated Manage returns to Home, while Categories-originated Manage returns to Categories. Escape and browser/native Back preserve the same hierarchy, and edited habit or category drafts prompt before discarding unsaved changes. Mobile Home, Categories, Manage, Insights, and Settings retain vertical scrolling while suppressing visible scrollbars and horizontal document movement.
