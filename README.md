...
**Your next wave** treats categories as navigation rather than timing rules. Shipped habits carry independent recommendation context: eligible start/end boundaries, an ideal interval, an optional flexible interval, an optional late form, genuine urgency where a real opportunity closes, indoor/outdoor setting, expected duration, and whether daylight is required. The runtime resolver can already accept one normalized per-habit preference window (`eligibleStart`, `idealStart`, `idealEnd`, `lateStart`, `eligibleEnd`) and fails closed to shipped defaults when that override is malformed; no preference-window setting is persisted or exposed in Manage yet. This is the seam for a future multi-user iteration rather than a hidden category rule.
