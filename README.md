...
Use the fixed **+** beside the Home category rail to open full-screen **Categories**. **All** is a permanent, protected aggregate and cannot be renamed, moved, assigned, archived, or reordered. Real categories can be created, renamed, assigned a curated icon, and reordered from their dedicated grips. Opening a category shows every assigned habit, regardless of today’s schedule or completion state. Habits move one at a time from their focused editor, retain their identity and history, and can be reordered only inside their real category. A category can be archived only after it has no assigned habits; restore returns it to its preserved position. Home keeps completed tracking cards grouped beneath **Completed · N**; it no longer offers a competing Reorder mode.
...
**Manage Habits** opens as a full-screen page in the category currently selected on Home. For example, opening Manage from Hygiene shows **Manage Hygiene** and only Hygiene habits, regardless of whether they are scheduled or complete today. Categories opens a protected **All habits** aggregate without a competing reorder model. Each habit opens a focused editor, where a category change is held until Save. Back, Escape, and browser/native Back unwind the full-screen hierarchy; an edited habit prompts before discarding unsaved changes.
